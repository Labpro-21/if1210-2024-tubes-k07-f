...
def upgrade_monster(currency, monsters, currentUser, mons, mInv, userpas, is_integer):
    display_monster(monsters)
    monster_choice = input(">>> Pilih monster: ")
    if is_integer(monster_choice):
        monster_choice = int(monster_choice) - 1
...
def LABORATORY(userpas, mInv, mons, currentUser, is_integer):
    ...
    while True:
        ...
        if choice == "1":
            currency, userpas, mInv = upgrade_monster(
                currency, monsters, currentUser, mons, mInv, userpas, is_integer)
        elif choice == "2":
